 ...
 * Requirement 1: Pharmacist approves a prescription.
 * Requirement 2: Pharmacist chooses physical dispenser slot: Slot 1, Slot 2, or Slot 3.
 * Requirement 3: After successful approval and slot assignment, generate unique one-time token.
 * Requirement 4: Token must be generated securely on server side (crypto, not Math.random()).
 * Requirement 6: Associated with prescription, authenticated patient, selected slot.
 * Requirement 7: Expiration time of 24 hours.
 * Requirement 8: Status initially 'issued'.
 */
 ...
 * Requirement 12: When patient clicks button, token claimed exactly once.
 * Requirement 13: Frontend must NOT directly update token status.
 * Requirement 14: Secure server-side atomic transition: issued → dispense_requested.
 * Requirement 15: If patient clicks twice, only the first request can succeed.
 * Requirement 16: If token is already dispense_requested, dispensed, expired, cancelled, rejected.
 * Requirement 17: Verify authenticated patient owns the token.
 * Requirement 18: Do NOT trust patient_id from browser.
 * Requirement 19: Determine patient identity from authenticated session.
 */
 ...
 * Requirement 10: Display active token in patient portal.
 */
 ...
 * Requirement 9: Display generated token in pharmacist portal.
 */
